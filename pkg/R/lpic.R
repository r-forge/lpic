lpic <- function(base = base, sd = sd, wt = wt, pics = "pics", preface = "f") {
  ...
  if(paste("Results_", as.character(Sys.Date()), "_", preface,  sep = "") %in% dir(getwd()) == TRUE) 
    stop("You already have a results file in your wd")  
  if(any(is.na(base$id)) != TRUE) stop("You do not have NAs in your id column")
  if(any(paste(base[,2], ".JPG", sep = "") %in% files == FALSE)) 
  ...
  if(any(sizes > 1000000) == TRUE) 
    print("The file size of >=1 of your photos is > 1mb, this could slow the program")
  rm(files, sizes)
  #summary of data, and a check to make sure everything looks alright
  print(paste("You have", length(colnames(base))-3, "measurements, and", 
    nrow(base), " total pictures. There are", nrow(base[is.na(base$id) == TRUE,]), 
    "pictures to assign ids.", sep = " "))
  ...
  matches <- do.call(rbind, lapply(1:nrow(base), function(i){
    base.2 <- do.call(rbind, lapply(1:nrow(main), function(e){base[i,]}))
    ...
    leng <- ncol(main)-1
    likelihood <- rowSums(do.call(cbind, lapply(4:ncol(main), function(j){
        dnorm(data[,j], data[,j+leng], sd[j-3])*wt[j-3] 
      })), na.rm = TRUE)
    #find max likelihood (i.e., likelihood when the pic is exactly the same)
    max.likel <- sum(do.call(c, lapply(4:ncol(main), function(j){
      dnorm(data[1,j], data[1,j], sd[j-3])*wt[j-3] 
      })), na.rm = TRUE)
    ...
    print(c(paste("Pic ", i, " of ", nrow(base), " : ", 
        ...
        "Specify by typing a string of 0s (for non-matches) and a 1 (for a match).",
        ...
        quote = FALSE)
    numbs <- as.numeric(unlist(strsplit(readline(prompt = ":"), split = "", fixed = TRUE)))
    if(length(numbs) != length(u)) print("You made an error, you have one more chance")
  ...
  #writing files into a folder in wd
  dir.create(paste(getwd(), "/Results_", d, "_", preface,  sep = ""))
  write.table(main, paste(getwd(),"/Results_", d, "_", preface, "/unique_ids.csv", 
      sep = ""), sep=",", row.names = FALSE, append = FALSE)
  write.table(matches, paste(getwd(),"/Results_", d, "_", preface, "/score.matrix.csv", 
      sep = ""), sep=",", row.names = FALSE, append = FALSE)
  #copying photo files into the folders
  dir.create(paste(getwd(), "/Results_", d, "_", preface, "/population",  sep = ""))
  u2 <- as.character(unique(main$id))
  for(x in 1:length(u2)){
    dir.create(paste(getwd(), "/Results_", d, "_", preface, "/population/", u2[x], sep = ""))
  }
  temp2 <- do.call(c, lapply(1:nrow(main), function(p){
    t <- file.copy(paste(getwd(), "/", pics, "/", main$photo_name[p], ".JPG", sep = ""), 
                   paste(getwd(), "/Results_", d, "_", preface, "/population/", main$id[p], "/", 
                   main$photo_name[p], ".JPG", sep = ""))
    return(t)
  }))
  if(any(temp2 == FALSE)) print("!Something went wrong with folder creation!")
  print("Complete")
